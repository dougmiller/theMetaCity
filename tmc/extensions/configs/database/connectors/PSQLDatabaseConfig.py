...
class PasswordNotFoundInPGPass(Exception):
    def __init__(self, host, port, name, user) -> None:
...
class PSQLDatabaseConfig:
    ...
    def __read_password(self) -> None:
        ...
        try:
            self.password = pgpasslib.getpass(self.host, int(self.port), self.name, self.user)
    ...
    def test_psql_connection(self) -> bool:
        """
        ...
        try:
            psycopg.connect(host=self.host, port=self.port, dbname=self.name, user=self.user, password=self.password)
